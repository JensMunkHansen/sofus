...
void MatrixArray(
  ...
  elements = element_array_create<T>(nRows*nCols, 1);
  ...
  for (size_t iRow=0 ; iRow < nRows ; iRow++) {
    // TODO(JEM): Implement move and copy assignment for sps::point_t<T>
...
void FocusedLinearArray(
  ...
  elements = element_array_create<T>(nElements, nSubH);
  ...
  if (arcPlacement == 0) {
    ...
    for (size_t iElement = 0 ; iElement < nElements ; iElement++) {
      for (size_t iSubH = 0 ; iSubH < nSubH ; iSubH++) {
        ...
        center[2] = - (cos(elAngle)*R - focus)  +  (R-focus);
        center[3] = T(0.0);  // For safety
        for (size_t iSubW = 0 ; iSubW < nSubW ; iSubW++) {
          sps::point_t<T> center1 = center;
          center1[0] +=
            T(2.0) * hw/T(nSubW) * (T(iSubW) - T(0.5)*(T(nSubW)-T(1.0)));
    ...
    for (size_t iElement = 0 ; iElement < nElements ; iElement++) {
      for (size_t iSubH = 0 ; iSubH < nSubH ; iSubH++) {
        ...
        for (size_t iSubW = 0 ; iSubW < nSubW ; iSubW++) {
          sps::point_t<T> center1 = center;
          center1[0] += T(2.0)*hw/T(nSubW)*(T(iSubW)-T(0.5)*(T(nSubW)-T(1.0)));
