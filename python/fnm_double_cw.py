...
xs = (np.r_[0:nx] - (nx-1.0)/2.0 + 0.5) * dx
...
pos = np.c_[xs.flatten(), np.zeros(nx*nz), zs.flatten()].astype(np.float64)
...
plt.figure()
result4 = out.reshape((nx,nz))

rho = 10000
omega = 2*np.pi*f0
pressure = -1j * omega * rho * np.exp(1j * omega * 0) * result4
